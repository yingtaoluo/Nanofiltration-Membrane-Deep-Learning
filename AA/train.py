...
parser.add_argument('data', type=str, choices=['0', '1', '2', '3', '4'],
...
if args.choice == 'rejection':
    batch_size, epoch = 200, 10
    learning_rate, regularization = 1e-3, 1e-3
    ...
    learning_rate, regularization = 1e-3, 3
    # manually pick the threshold to save the model parameters
    threshold = 0
...
'''loading data'''
train_numpy_data = np.load(dps + "/train_dataset.npy")  # size (320000,446)
test_numpy_data = np.load(dps + "/test_dataset.npy")  # size (29290, 446)
valid_numpy_data = np.load(dps + "/valid_dataset.npy")  # size (70700, 446)
np.random.shuffle(train_numpy_data)
np.random.shuffle(test_numpy_data)
np.random.shuffle(valid_numpy_data)
...
train_input = torch.FloatTensor(train_numpy_data[:, 0:444]).to(args.device)
if args.choice == 'rejection':
    train_label = torch.FloatTensor(train_numpy_data[:, 445:446]).to(args.device)
elif args.choice == 'permeability':
    train_label = torch.FloatTensor(train_numpy_data[:, 444:445]).to(args.device)
else:
    raise NotImplementedError()
train_data = data.TensorDataset(train_input, train_label)
train_loader = data.DataLoader(dataset=train_data, batch_size=batch_size, shuffle=True)

test_input = torch.FloatTensor(test_numpy_data[:, 0:444]).to(args.device)
if args.choice == 'rejection':
    test_label = torch.FloatTensor(test_numpy_data[:, 445:446]).to(args.device)
elif args.choice == 'permeability':
    test_label = torch.FloatTensor(test_numpy_data[:, 444:445]).to(args.device)
else:
    raise NotImplementedError()

valid_input = torch.FloatTensor(valid_numpy_data[:, 0:444]).to(args.device)
if args.choice == 'rejection':
    valid_label = torch.FloatTensor(valid_numpy_data[:, 445:446]).to(args.device)
elif args.choice == 'permeability':
    valid_label = torch.FloatTensor(valid_numpy_data[:, 444:445]).to(args.device)
else:
    raise NotImplementedError()
